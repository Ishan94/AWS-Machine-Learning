...
def test(model, test_loader, criterion, device, epoch_num):
    
    logger.info(f"Epoch: {epoch_num} - Performing Model Testing on Testing Dataset")
    model.eval()
    running_loss = 0
    running_corrects = 0
    with torch.no_grad(): 
...
def train(model, train_loader, criterion, optimizer, device, epoch_num):
...
def main(args):
    ...
    for epoch_num in range(1, args.epochs +1):
        ...
        model=train(model, train_data_loader, loss_criterion, optimizer, device, epoch_num)
        logger.info(f"Epoch {epoch_num} - Starting Testing phase.")
        test(model, test_data_loader, loss_criterion, device, epoch_num)
